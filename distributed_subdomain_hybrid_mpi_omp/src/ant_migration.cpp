...
// Updates next pheromone values for one owned local interior cell.
inline void mark_local_cell_from_current(const DomainDecomposition& decomp, const StepContext& ctx,
                                         std::size_t local_idx) {
    const int stride = decomp.stride_x();
    const std::size_t left = local_idx - 1;
    const std::size_t right = local_idx + 1;
    const std::size_t up = local_idx - static_cast<std::size_t>(stride);
    const std::size_t down = local_idx + static_cast<std::size_t>(stride);

    const double v1_left = std::max(0.0, ctx.cur_v1[left]);
    const double v1_right = std::max(0.0, ctx.cur_v1[right]);
    const double v1_up = std::max(0.0, ctx.cur_v1[up]);
    const double v1_down = std::max(0.0, ctx.cur_v1[down]);

    const double v2_left = std::max(0.0, ctx.cur_v2[left]);
    const double v2_right = std::max(0.0, ctx.cur_v2[right]);
    const double v2_up = std::max(0.0, ctx.cur_v2[up]);
    const double v2_down = std::max(0.0, ctx.cur_v2[down]);
    ...
    ctx.next_v1[local_idx] = ctx.alpha * max_v1 + (1.0 - ctx.alpha) * avg_v1;
    ctx.next_v2[local_idx] = ctx.alpha * max_v2 + (1.0 - ctx.alpha) * avg_v2;
}

inline std::size_t local_idx_from_owned_global(const DomainDecomposition& decomp, int gx, int gy) {
    const int lx = decomp.local_x_from_global(gx);
    const int ly = decomp.local_y_from_global(gy);
    return decomp.idx(lx, ly);
}
...
inline void mark_cell_from_current(const DomainDecomposition& decomp, const StepContext& ctx, int gx, int gy) {
    ...
    mark_local_cell_from_current(decomp, ctx, local_idx_from_owned_global(decomp, gx, gy));
}

// Advances one local ant until local budget is consumed or the ant migrates out.
int process_local_ant(const DomainDecomposition& decomp, const StepContext& ctx, int& ant_x, int& ant_y, std::uint8_t& ant_loaded, std::uint64_t& ant_seed, std::size_t& local_food_counter, int my_rank, double& consumed_out, std::vector<std::size_t>* touched_cells) {
    consumed_out = 0.0;
    ...
    while (consumed_out < 1.0) {
        ...
        if (owner == my_rank) {
            if (touched_cells != nullptr) {
                touched_cells->push_back(local_idx_from_owned_global(decomp, ant_x, ant_y));
            } else {
...
StepResult advance_ants_with_migration(const DomainDecomposition& decomp, const StepContext& ctx, Population& ants, MPI_Comm comm) {
    ...
    std::vector<NeighborQueues> outgoing_by_thread(static_cast<std::size_t>(thread_count));
    std::vector<std::vector<TransitAnt>> finished_by_thread(static_cast<std::size_t>(thread_count));
    std::vector<std::vector<std::size_t>> touched_by_thread(static_cast<std::size_t>(thread_count));
    std::vector<std::size_t> food_by_thread(static_cast<std::size_t>(thread_count), 0);

    const std::size_t local_cap = ants.size() / static_cast<std::size_t>(thread_count) + 8;
    for (int tid = 0; tid < thread_count; ++tid) {
        reserve_neighbor_queues(outgoing_by_thread[static_cast<std::size_t>(tid)], local_cap / 8 + 8);
        finished_by_thread[static_cast<std::size_t>(tid)].reserve(local_cap);
        touched_by_thread[static_cast<std::size_t>(tid)].reserve(local_cap * 2);
    }
...
#pragma omp parallel
    {
        ...
        auto& local_outgoing = outgoing_by_thread[static_cast<std::size_t>(tid)];
        auto& local_finished = finished_by_thread[static_cast<std::size_t>(tid)];
...
#pragma omp for schedule(static)
        for (std::size_t i = 0; i < ants.size(); ++i) {
            ...
            const int owner =
                process_local_ant(decomp, ctx, ant_x, ant_y, ant_loaded, ant_seed, local_food_counter, decomp.rank,
                                  consumed, &local_touched);

            if (owner == decomp.rank) {
                local_finished.push_back(TransitAnt{ant_x, ant_y, ant_seed, 0.0, ant_loaded, 0});
            } else if (owner != MPI_PROC_NULL) {
                route_ant_packet(decomp, TransitAnt{ant_x, ant_y, ant_seed, consumed, ant_loaded, 1}, owner,
                                 local_outgoing);
            }
        }

        food_by_thread[static_cast<std::size_t>(tid)] = local_food_counter;
    }
#else
    {
        auto& local_outgoing = outgoing_by_thread[0];
        auto& local_finished = finished_by_thread[0];
        ...
        food_by_thread[0] = local_food_counter;
    }
#endif

    std::size_t total_food = 0;
    std::size_t total_up = 0;
    std::size_t total_down = 0;
    std::size_t total_left = 0;
    std::size_t total_right = 0;
    for (int tid = 0; tid < thread_count; ++tid) {
        const std::size_t idx = static_cast<std::size_t>(tid);
        total_food += food_by_thread[idx];
        total_up += outgoing_by_thread[idx].up.size();
        total_down += outgoing_by_thread[idx].down.size();
        total_left += outgoing_by_thread[idx].left.size();
        total_right += outgoing_by_thread[idx].right.size();
    }
    result.food_collected_local += total_food;

    outgoing.up.reserve(std::max(outgoing.up.capacity(), total_up));
    outgoing.down.reserve(std::max(outgoing.down.capacity(), total_down));
    outgoing.left.reserve(std::max(outgoing.left.capacity(), total_left));
    outgoing.right.reserve(std::max(outgoing.right.capacity(), total_right));

    for (int tid = 0; tid < thread_count; ++tid) {
        const std::size_t idx = static_cast<std::size_t>(tid);
        for (const std::size_t local_idx : touched_by_thread[idx]) {
            mark_local_cell_from_current(decomp, ctx, local_idx);
        }
        for (const TransitAnt& ant : finished_by_thread[idx]) {
            finished.add_ant(position_t{ant.x, ant.y}, static_cast<std::size_t>(ant.seed), ant.loaded);
        }
        outgoing.up.insert(outgoing.up.end(), outgoing_by_thread[idx].up.begin(), outgoing_by_thread[idx].up.end());
        outgoing.down.insert(outgoing.down.end(), outgoing_by_thread[idx].down.begin(),
                             outgoing_by_thread[idx].down.end());
        outgoing.left.insert(outgoing.left.end(), outgoing_by_thread[idx].left.begin(),
                             outgoing_by_thread[idx].left.end());
        outgoing.right.insert(outgoing.right.end(), outgoing_by_thread[idx].right.begin(),
                              outgoing_by_thread[idx].right.end());
    }
