...
static advance_timing advance_time(
...
static void write_summary_metric(
    std::ostream& out, const std::string& metric_name, const running_stats& stats
) {
    out << metric_name << ","
        << stats.count << ","
        << stats.sum << ","
...
int main(int argc, char* argv[])
{
    ...
    std::size_t seed = 2026; // Seed for random generation (reproducible)
    const std::size_t nb_ants = opts.nb_ants; // Number of ants
    const double eps = 0.8;  // Exploration coefficient
    const double alpha = 0.7; // Noise coefficient
    //const double beta=0.9999; // Evaporation coefficient
    ...
    // Generate the terrain 512 x 512 (2*(2^8) per direction)
    ...
    /* Rescale fractal_land values so they are in the [0, 1] range */
    for (fractal_land::dim_t i = 0; i < land.dimensions(); ++i)
        for (fractal_land::dim_t j = 0; j < land.dimensions(); ++j) {
            land(i,j) = (land(i,j)-min_val)/delta;
        }

    // Set the exploration coefficient for all ants.
    ant::set_exploration_coef(eps);
    // Create ants at random positions on the map:
    std::vector<ant> ants;
    ...
    // Counter for food units brought back to the nest by ants.
        ...
        if (it > opts.warmup_iterations) {
            ...
            if (timing_csv) {
                timing_csv
                    ...
                    << event_poll_ms << ","
                    << sim_timing.ants_ms << ","
                    << sim_timing.evaporation_ms << ","
                    << sim_timing.update_ms << ","
                    << sim_timing.total_ms << ","
                    << render_ms << ","
                    << blit_ms << ","
                    << iteration_total_ms << "\n";
