...
void Population::advance_all(pheronome& phen, const fractal_land& land,
    ...
    std::vector<std::vector<pheronome::size_t>> touched_by_thread(
...
#pragma omp for schedule(static)
    ...
    // Apply all pheromone marks after the parallel region (deferred marking).
    // All marks are derived from the same pheromone snapshot, so this is safe.
...
void Population::advance_one(std::size_t idx, pheronome& phen, const fractal_land& land,
                             const position_t& pos_food, const position_t& pos_nest,
    ...
    while (consumed_time < 1.0) {
        ...
        const double left_val = phen.value(old_flat - stride, ind_pher);
        const double right_val = phen.value(old_flat + stride, ind_pher);
        const double up_val = phen.value(old_flat - 1, ind_pher);
        const double down_val = phen.value(old_flat + 1, ind_pher);
        const double max_phen = std::max({left_val, right_val, up_val, down_val});
        ...
        if ((choice > m_eps) || (max_phen <= 0.0)) {
            ...
            if (left_val == max_phen) {
                new_x -= 1;
                new_flat = old_flat - stride;
            } else if (right_val == max_phen) {
                new_x += 1;
                new_flat = old_flat + stride;
            } else if (up_val == max_phen) {
                new_y -= 1;
                new_flat = old_flat - 1;
            } else {
                new_y += 1;
