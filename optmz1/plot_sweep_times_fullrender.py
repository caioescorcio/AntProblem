...
Expected input files:
  results/sweeps_render_full_food/summary_threads{P}_ants{N}.csv
...
SUMMARY_RE = re.compile(r"summary_(?:threads|ranks|procs)(?P<procs>\d+)_ants(?P<ants>\d+)\.csv$")
...
def mpi_compute_comm(metrics: Dict[str, float]) -> float:
    return (
        metric(metrics, "mpi_pheromone_sync")
        + metric(metrics, "mpi_evaporation_sync")
        + metric(metrics, "mpi_food_sync")
    )
...
def plot_total_for_ant(data: SweepData, ants: int, out_path: Path, dpi: int) -> None:
    ...
    ax.set_title(f"MPI Population Decomposition Breakdown (ants={ants})")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    default_results = root / "results" / "sweeps_render_full_food"
    default_out = default_results / "plots"

    parser = argparse.ArgumentParser(description="Plot optmz1 MPI sweep timing summaries.")
    parser.add_argument("--results-dir", type=Path, default=Path("src/results/mpi_sweep"), help="Default: src/results/mpi_sweep")
...
def main() -> int:
    ...
    out_speedup_table = args.output_dir / "mpi_speedup_table.tex"
    out_efficiency_table = args.output_dir / "mpi_efficiency_table.tex"
    table_files = write_speedup_efficiency_tables(data, out_speedup_table, out_efficiency_table)
    generated.extend(table_files)

    print("Generated plots and tables:")
